...
#
# bt-87c5: this used to be `crontab -l 2>/dev/null | grep -q "..."` directly.
# grep -q exits the instant it matches, closing its end of the pipe; crontab -l
# on this box emits ~70KB against a 64KB pipe buffer, so a still-writing
# crontab -l can take that early close as SIGPIPE, which `pipefail` turns into
# a non-zero pipeline exit -- on the ONE input this guard exists to catch (the
# watchdog IS in crontab), the `if` could read FALSE and print OK. Capturing
# via command substitution first means bash itself drains crontab -l to EOF
# (no early-closing reader), and testing the capture with a here-string means
# grep never has a pipe to SIGPIPE either.
crontab_live="$(crontab -l 2>/dev/null || true)"
if grep -q "check-heartbeat-liveness.sh" <<<"$crontab_live"; then
...
# --- 7b. bt-87c5 regression test for the SIGPIPE-fails-open bug above --
#         stub `crontab` on PATH so `-l` emits the watchdog line immediately
#         followed by >64KB of filler generated by `yes` (a fast, still-going
#         producer at the moment grep would exit -- a `cat` of a small file
#         finishes writing too quickly to reliably race). Never touches the
#         real crontab.
FAKE_CRONTAB_BIN_87C5="$TMP/fakebin-bt-87c5"
mkdir -p "$FAKE_CRONTAB_BIN_87C5"
cat > "$FAKE_CRONTAB_BIN_87C5/crontab" <<'FAKECRONTABEOF'
#!/usr/bin/env bash
if [[ "$1" == "-l" ]]; then
  echo "* * * * * $REPO/scripts/check-heartbeat-liveness.sh"
  # Deliberately NOT wrapped with an `exit 0` -- this stub's own exit status
  # must be free to reflect a SIGPIPE hitting the `head` below (its write is
  # the one touching the real outer pipe to the caller's grep), otherwise the
  # repro can't reproduce anything: an explicit `exit 0` here would swallow
  # that and always report success regardless of what happened upstream.
  yes "# bt-87c5 sigpipe-repro filler padding past the 64KB pipe buffer" | head -c 200000
  exit
fi
exit 1
FAKECRONTABEOF
chmod +x "$FAKE_CRONTAB_BIN_87C5/crontab"

# (a) prove the vulnerability class this fix removes is real: the OLD
#     direct-pipe shape can wrongly read the watchdog as absent even though
#     the stub crontab lists it front-and-center.
old_pipe_false_negative=0
for attempt in 1 2 3; do
  if ! REPO="$REPO" PATH="$FAKE_CRONTAB_BIN_87C5:$PATH" bash -c 'set -uo pipefail; crontab -l 2>/dev/null | grep -q "check-heartbeat-liveness.sh"'; then
    old_pipe_false_negative=1
    break
  fi
done
if [[ "$old_pipe_false_negative" -eq 1 ]]; then
  echo "OK     bt-87c5 repro: confirmed the old 'crontab -l | grep -q' shape CAN miss the watchdog in crontab (SIGPIPE under pipefail) -- this is the bug the fix above removes"
else
  echo "FAIL   bt-87c5 repro: could not reproduce the old pipe's SIGPIPE false-negative in 3 attempts against the >64KB stub -- can't demonstrate what the fix guards against"
  FAIL=1
fi

# (b) the fixed guard (capture via command substitution, test with a
#     here-string -- neither side is a pipe, so nothing can SIGPIPE) must
#     catch the same stub every time.
fixed_guard_missed=0
for attempt in 1 2 3; do
  repro_live="$(REPO="$REPO" PATH="$FAKE_CRONTAB_BIN_87C5:$PATH" crontab -l 2>/dev/null || true)"
  if ! grep -q "check-heartbeat-liveness.sh" <<<"$repro_live"; then
    fixed_guard_missed=1
    break
  fi
done
if [[ "$fixed_guard_missed" -eq 0 ]]; then
  echo "OK     bt-87c5: fixed guard (capture + here-string) caught the watchdog in the >64KB stub crontab on 3/3 tries"
else
  echo "FAIL   bt-87c5: fixed guard missed the watchdog in the >64KB stub crontab at least once"
  FAIL=1
fi
