...
## Log
- 2026-08-01 dropped: Duplicate of bt-d30a, which pre-existed it and is now done (85b3ee1 — step 5 polls crontab -l 5x before failing). Main filed this from the bt-3f5a worker's follow-up text without grepping the board first, which is exactly the check that prevents duplicate filing. The worker's own report even named bt-d30a as already-known; I boarded it anyway. No work lost — the flake is fixed.
